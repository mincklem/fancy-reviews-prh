...
function nlp() {
   ...
   $("#textual_analysis_tab").click(function() {
    ...
    //adjectives
    $("#npl_adj").click(function() {
        var adj = window.nlp.pos(""+reviewTextString+"").adjectives()
            console.log(adj);
          });
    //ngrams
    $("#npl_ngram").click(function() {
        $(".ngram_table").remove();
        ...
        var x = 7;
            $.each(cut_gram, function(){
                x = x-1;
                $("#ngrams").append("<table class='table table-hover ngram_table' id='ngram_table"+x+"'><strong><caption>"+x+"-term Phrases</caption><thead><tr><th data-sort='string'>Phrase</th><th data-sort='float'>Freq</th><th data-sort='float'>Avg Stars</th><th data-sort='float'>Diff from Avg</th></strong></tr></thead></table>");
...
function starSubmit(){
    $("#starSubmit").click(function(){
        ...
        $(".fa-pulse").css("visibility", "visible");
        $("#cloudNotify").css("visibility", "visible");
        $(".summaryOverlay").css("visibility", "visible");
        $(".summaryOverlay").css("z-index", "2");
        //star checking
         checkCloudTerms();
...
function makeCloud(terms) {
    ...
    for(var i = 0; i < data_labels.length; i++) {  // take every second element
            ...
            if (data_star_avgs[i]) {
               $("#term_table"+n+"").append("<tr><td>"+data_labels[i]+"</td><td>"+data_vals[i]+"</td><td>"+percent_of_terms+"%</td><td>"+data_star_avgs[i].toFixed(2)+"</td></tr>"); 
           } else {
            $("#term_table"+n+"").append("<tr><td>"+data_labels[i]+"</td><td>"+data_vals[i]+"</td><td>"+percent_of_terms+"%</td><td></td></tr>");
            }
